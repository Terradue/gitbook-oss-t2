...
#repo.frame <- repo.frame[order(repo.frame$Section,repo.frame$ProjectName),]

#sectionList <- unlist(unique(repo.frame$Section))

# build the page with reference link 
template <- "template/repo.Rmd"
for(i in sectionClass)
{
     section.frame <- repo.frame[section==i, ]
     section.frame <- section.frame[order(section.frame$ProjectName), ]
     
     cat(paste0("* [", i, "](chapter/", gsub(" ", "-", i), ".md)\n"))
  
     for(j in nrow(section.frame))
     {
          repo <- gsub(" ", "-", section.frame[j]$ProjectName)
       
          repo.declaration <- paste0("repo <- '", repo, "'")
       
          cat(paste0(" * [", i, "](section/", repo, ".md)\n"))
